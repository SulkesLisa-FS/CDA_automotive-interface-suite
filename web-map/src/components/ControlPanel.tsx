...
export default function ControlPanel({
  ...
  const [newAlert, setNewAlert] = useState({
    ...
    coordinates: [-84.3880, 33.7490] as [number, number], // Atlanta, GA
    message: '',
  ...
  const handleCreateAlert = () => {
    ...
    setNewAlert({
      ...
      message: '',
      coordinates: [
        -84.3880 + (Math.random() - 0.5) * 0.02, // Random location around Atlanta
  ...
  const handleWeatherChange = (condition: string) => {
    ...
    // Adjust speed limits based on weather
  ...
  const handleEmergencyMode = () => {
    ...
    if (newEmergencyMode) {
      onAlertCreate({
        type: 'emergency',
        coordinates: [-84.3880, 33.7490], // Atlanta center
