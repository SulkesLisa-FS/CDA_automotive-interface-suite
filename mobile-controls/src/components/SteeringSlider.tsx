...
const KNOB_SIZE = 50;
export default function SteeringSlider({ onSteer, disabled = false }: SteeringSliderProps) {
  ...
  const panResponder: PanResponderInstance = PanResponder.create({
    ...
    onPanResponderMove: (evt, gestureState) => {
      // Use absolute touch position instead of relative dx
  ...
  // Listen to animation updates and update steering value during animation
  useEffect(() => {
    const listener = animatedPosition.addListener(({ value }) => {
      if (!isDragging) {
        setKnobPosition(value);
        
        // Calculate steering value during animation
        const centerX = SLIDER_WIDTH / 2;
        const maxDistance = (SLIDER_WIDTH - KNOB_SIZE) / 2;
