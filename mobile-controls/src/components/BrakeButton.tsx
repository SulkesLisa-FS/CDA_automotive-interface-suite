...
export default function BrakeButton({ onBrake, disabled = false }: BrakeButtonProps) {
  ...
  const startBraking = () => {
    ...
    // Haptic feedback (iOS) or vibration (Android)
    if (Platform.OS === 'ios') {
      // iOS haptic feedback would go here if using expo-haptics
    } else {
    ...
    pressTimer.current = setInterval(() => {
      currentForce = Math.min(1, currentForce + 0.1); // Increase to max 1.0
