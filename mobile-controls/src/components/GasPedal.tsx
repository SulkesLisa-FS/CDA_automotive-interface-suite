...
export default function GasPedal({ onThrottle, disabled = false }: GasPedalProps) {
  const [knobPosition, setKnobPosition] = useState(PEDAL_HEIGHT - KNOB_SIZE / 2); // Start at bottom (0%)
  ...
  const panResponder: PanResponderInstance = PanResponder.create({
    ...
    onPanResponderMove: (evt, gestureState) => {
      // Use absolute touch position instead of relative dy
      const touchY = evt.nativeEvent.locationY;
      
      // Calculate new knob position - follow touch directly!
      const minY = KNOB_SIZE / 2;
      const maxY = PEDAL_HEIGHT - KNOB_SIZE / 2;
      const newPosition = Math.max(minY, Math.min(maxY, touchY));
      
      // Update position immediately - no resistance!
      setKnobPosition(newPosition);
      animatedPosition.setValue(newPosition);
      
      // Convert to throttle value (0 to 1, inverted because bottom=0, top=1)
  ...
  // Listen to animation updates and update throttle value during animation
  useEffect(() => {
    const listener = animatedPosition.addListener(({ value }) => {
      if (!isDragging) {
        setKnobPosition(value);
        
        // Calculate throttle value during animation
        const minY = KNOB_SIZE / 2;
        const maxY = PEDAL_HEIGHT - KNOB_SIZE / 2;
