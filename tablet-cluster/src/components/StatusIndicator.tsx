...
export default function StatusIndicator({ 
  ...
  useEffect(() => {
    if (active && blinking) {
      ...
      Animated.loop(
        Animated.sequence([
          Animated.timing(opacityAnimation, { toValue: 1, duration: 500, useNativeDriver: true }),
          Animated.timing(opacityAnimation, { toValue: 0.2, duration: 500, useNativeDriver: true })
        ])
      ).start();
    } else if (active) {
      ...
      Animated.sequence([
        ...
        Animated.timing(scaleAnimation, { toValue: 1.0, duration: 150, useNativeDriver: true })
      ]).start();
    } else {
      // Off state
